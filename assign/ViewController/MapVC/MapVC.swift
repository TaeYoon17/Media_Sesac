...
//        mapView.gestureRecognizerShouldBegin()
        passthroughNavTabHidden.debounce(for: .seconds(0.6), scheduler: DispatchQueue.main).sink { val in
            self.navigationController?.setNavigationBarHidden(val, animated: true)
            DispatchQueue.main.async { self.tabBarController?.tabBar.isHidden = val }
        }.store(in: &subscription)
    }
    var passthroughNavTabHidden = PassthroughSubject<Bool,Never>()
    var isNavTabHidden = false{
        didSet{
            guard isNavTabHidden != oldValue else {return}
            passthroughNavTabHidden.send(isNavTabHidden)
        }
    }
    @objc func mapTapped(){
        print("isTapped!!")
        isNavTabHidden.toggle()
    }
